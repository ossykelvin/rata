...
test('mock agent executes calculator through policy and reports missing skill tools honestly', async () => {
  ...
  // This used to ask about scanning a drive, then about the screen, each time
  // the previous honest-refusal fixture gained its tools. RATA-015 registered
  // screen.capture and vision.analyze, so the path needs a skill that is still
  // blocked: `email-assistant` needs mail tools, which are a different lane.
  const missing = await agent.handle('Find the latest email from...')
  assert.match(missing.message, /not registered yet/i)
  assert.match(missing.message, /mail\.(search|read)/)
