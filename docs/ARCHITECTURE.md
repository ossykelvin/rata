...
`sleeping` is also a presentation state. The renderer maps those events in `src/components/character/`; it does not choose tools. Temporary art is the original concept-sheet crop in `public/rata-concept.png`. Missing assets fall back to a letter-mark silhouette.
